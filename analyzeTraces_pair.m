%% analyzeTraces_pair
% this program is a version of the analyzeTraces program that is
% specialized for paired recordings.
% the user imports two traces (postsynaptic evoked responses and
% presynaptic action potentials).
% event selection is locked to action potential peaks.

function analyzeTraces_pair
    % author: Bryce Grier 
...
%% initialize shared variables
    ...
    secondaryTraceSamples = [];
    secondaryTracePeakLocs = [];
...
%% create navigation tab
    ...
    % list boxes containing experiments, recording files, and existing analysis files
        ...
        uilabel('Parent',navigationTab,...
            ...
            'HorizontalAlignment','Center',...
            'fontweight', 'bold',...
            'Position',[navigationTabWidth*0.4-(175/2) navigationTabHeight*0.775 175 40]);
        mainTraceControl = uilistbox('Parent',navigationTab,...
            'Items',traceliststrings,...
            'Position',[navigationTabWidth*0.4-(150/2) navigationTabHeight*0.55-(125/2) 150 125]);
        
        uilabel('Parent',navigationTab,...
            'Text',sprintf('%s\n%s','Choose a secondary trace',' to begin analysis'),...
            'HorizontalAlignment','Center',...
            'fontweight', 'bold',...
            'Position',[navigationTabWidth*0.6-(175/2) navigationTabHeight*0.775 175 40]);
        secondaryTraceControl = uilistbox('Parent',navigationTab,...
            'Items',traceliststrings,...
            'Position',[navigationTabWidth*0.6-(150/2) navigationTabHeight*0.55-(125/2) 150 125]);
...
%% create settings tab
    ...
    % radio buttons for average trace alignment preference
        traceButtonWidth = settingsTabWidth*0.175;
        traceButtonHeight = settingsTabHeight*0.15;
        traceAlignButton  = uibuttongroup('Parent',settingsTab,...
            'Title','Average Trace Alignment',...
            'fontweight','bold',...
            'SelectionChangedFcn',@alignmentChange,...
            'Position',[settingsTabWidth*0.2-(160/2) settingsTabHeight*0.80-(70/2) traceButtonWidth traceButtonHeight]);
    ...
    % controls for setting latency threshold    
        uilabel('Parent',settingsTab,...
            'Text','Latency Threshold (ms) =',...
            'fontweight', 'bold',...
            'HorizontalAlignment','Center',...
            'Position',[settingsTabWidth*0.45-(150/2) settingsTabHeight*0.8 150 20]);
        latencyField = uieditfield(settingsTab,...
            'numeric',...
            'Value',latencyThreshold,...
            'ValueChangedFcn',@latencyUpdate,...
            'Position',[settingsTabWidth*0.55-(30/2) settingsTabHeight*0.8 40 20],...
            'HorizontalAlignment','Center');
...
%% create analysis tab
    ...
    uibutton('Parent',analysisTab,...
        'Text',sprintf('%s\n%s','Mulit-peak','Sort [f]'),...
        'ButtonPushedFcn',@addToMultiGroup,...
...
%         currentWindow = 1:windowScope;
        ...
        secondaryFileName = secondaryTraceControl.Value;
        if fileName ~= ""
        ...
        spikeIdx = 1;       
        nextSamplePoint = secondaryTracePeakLocs(spikeIdx);
        newCurrentWindowBegin = nextSamplePoint-floor(windowScope/2)+1;
        newCurrentWindowEnd = nextSamplePoint+floor(windowScope/2);
    ...
    % resume analysis that has been previously worked on and saved
        ...
        load(analysisList.Value, 'dataTable', 'currentWindow', 'selectedEvents', 'traceSamples',...
            'decayStartPercent','decayEndPercent','riseStartPercent','riseEndPercent','RMS','samplesPerMilliSecond',...
            'preEventSamples','postEventSamples','autoZoom','secondaryTraceSamples');
        
        % find speaks in secondary trace
        [~,secondaryTracePeakLocs] = findpeaks(secondaryTraceSamples(:,1),'MinPeakHeight',0,'MinPeakDistance',200);
    ...
    function importSecondaryTrace
    % import a secondary recording trace
        
        delimiter = {''};
        startRow = 2;
        formatSpec = '%f%[^\n\r]';
        fileID = fopen(secondaryFileName,'r');
        dataArray = textscan(fileID, formatSpec, 'Delimiter', delimiter,...
            'TextType', 'string','EmptyValue', NaN, 'HeaderLines' ,startRow-1,...
            'ReturnOnError', false, 'EndOfLine', '\r\n');
        fclose(fileID);
        secondaryTraceSamples = [dataArray{1:end-1}];
        
        % add auxiliary columns
        for i = 1:size(secondaryTraceSamples,traceValueCol)
            secondaryTraceSamples(i,traceTimeCol) = i;
        end
...
%% selection functions
    ...
    % allows user to click on event in the data table and move to that event
        ...
        spikeIdx = sum(secondaryTracePeakLocs < eventThresholdTime);      
        plotLocation;
    end
...
%             xlim(tracePlot,[eventTime-50 eventTime+50]);
            yRangeLims = traceSamples(eventTime-100:eventTime+100,traceValueCol) - meanWindowY;
            yRange = range(traceSamples(eventTime-100:eventTime+100,traceValueCol));
%             ylim(tracePlot,'auto');
...
%             ylim(tracePlot,'auto');
    ...
    % plot the current view window, as well as any analyzed events that reside within
        ...
        % generate and plot x and y values of secondary trace
        traceX = secondaryTraceSamples(currentWindow,traceTimeCol);
        traceY = secondaryTraceSamples(currentWindow,traceValueCol);
        maxY = max(secondaryTraceSamples(:,traceValueCol));
        traceY = traceY - maxY;
        plot(tracePlot,traceX,traceY,'Color',[0.5 0.5 0.5]);
        localPeaks = intersect(traceX,secondaryTracePeakLocs);
        if ~isempty(localPeaks)
            for peakIdx = 1:length(localPeaks) 
                xline(tracePlot,localPeaks(peakIdx)+(latencyThreshold*samplesPerMilliSecond),'c');
            end
        end      
        ...
        locationY = yMin+ceil(0.025*yRange)-yOffset;
    ...
    % update the table that displays measurements of events
        ...
        avgTraceCol(avgTraceLogical) = {char('X')};
        ...
        dataTable = table((1:sum(eventsForTableLogical))',...
            ...
            selectedEventsSorted(:,latencyValueCol),...
            'VariableNames',...
                {'Event',...
                sprintf('Average\nTrace'),...
                sprintf('Full\nGroup'),...
        ...
        set(uit, 'ColumnWidth',{'4x','5x','4x','4x','4x','4x','5x','4x','5x','4x','4x','4x'});
...
%% UI buttons
    ...
    % add event to group in which the full event is measured
        ...
        % determine if event is within the desired latency
        possiblePeaks = secondaryTracePeakLocs(secondaryTracePeakLocs < eventThresholdTime);
        closestPeak = possiblePeaks(end);
        eventLatency = eventThresholdTime - closestPeak;
        if eventLatency > latencyThreshold*samplesPerMilliSecond
            uialert(mainPanel, 'Event does not occur between peak and latency threshold.','');
            return;
        end
        ...
        if eventThresholdTime - preEventSamples < 1
            uialert(mainPanel, 'Event is too close to beginning of recording to be in average trace.','');
            selectedEvents(eventIndex,averageTraceLogicalCol) = nan;
%             return;
        elseif eventPeakTime + postEventSamples > length(traceSamples)
            uialert(mainPanel, 'Event is too close to end of recording to be in average trace.','');
            selectedEvents(eventIndex,averageTraceLogicalCol) = nan;
%             return;
        end        
    ...
    % add event to group in which only the rise phase of an event is measured
        ...
        eventLatency = eventThresholdTime - closestPeak;
        if eventLatency > latencyThreshold*samplesPerMilliSecond
            uialert(mainPanel, 'Event is too far from a spike.','');
            return;
        end
    ...
    % add event to group in which only the rise phase of an event is measured
        ...
        selectedEvents(eventIndex,latencyValueCol) = eventLatency/samplesPerMilliSecond;
    ...
    % add event to group in which only the rise phase of an event is measured
        ...
        selectedEvents(eventIndex,aucValueCol) = abs(eventRiseAUC + eventDecayAUC);
        selectedEvents(eventIndex,amplitudeValueCol) = eventPeakValue - eventThresholdValue;
        selectedEvents(eventIndex,multiLogicalCol) = 1;
        selectedEvents(eventIndex,frequencyLogicalCol) = 1;
        selectedEvents(eventIndex,eventTimeCol) = eventThresholdTime;
...
%             return;
%         end
%         [~,~,prevIdx] = intersect(currentWindow,secondaryTracePeakLocs);
        if spikeIdx-1 < 1
            uialert(mainPanel, 'Beginning of trace.','');
            plotLocation;
            return;
        ...
        spikeIdx = spikeIdx - 1;
        prevSamplePoint = secondaryTracePeakLocs(spikeIdx);
        newCurrentWindowBegin = prevSamplePoint-floor(windowScope/2)+1;
        newCurrentWindowEnd = prevSamplePoint+floor(windowScope/2);
        currentWindow = newCurrentWindowBegin:newCurrentWindowEnd;
        if (currentWindow(end) > size(traceSamples,1))
            currentWindow = traceSamples(end-windowScope+1:end,traceTimeCol)';
        elseif (currentWindow(1) < 1)
...
%         [peakLocs,~,nextIdx] = intersect(currentWindow,secondaryTracePeakLocs); 
        if spikeIdx + 1 > length(secondaryTracePeakLocs)
            uialert(mainPanel, 'End of trace.','');
            plotLocation;
            return;
        end           
    ...
    % save a .mat file containing relevant analysis data for later use
        ...
        save(saveName, 'dataTable', 'currentWindow', 'selectedEvents', 'traceSamples',...
            ...
            'RMS','samplesPerMilliSecond',...
            'preEventSamples','postEventSamples','autoZoom',...
            'secondaryTraceSamples');
        d.Value = 0.9;
    ...
    % cycle through selectedEvents to re-analyze events with different settings
        ...
        for updateIndex = 1:sum(~isnan(selectedEvents(:,eventTimeCol)),1) 
            eventIndex = length(selectedEvents);
            eventCurrentlySelected = true;
...
%% helper functions
    ...
    % remove evetns that fall below new noise threshold
        ...
        latencyThreshold = latencyField.Value;
        
        % prompt user to remove events and act on decision
        decisionLatency = uiconfirm(mainPanel,...
            'Remove events that exceed the new threshold?','',...
            'Options',{'Yes','No'});
        if strcmp(decisionLatency,'Yes')
            for checkIndex = 1:size(selectedEvents,1)
                if abs(selectedEvents(checkIndex,latencyValueCol)) > latencyThreshold
                    eventThresholdTime = selectedEvents(checkIndex,eventTimeCol);
                    selectedEvents(checkIndex,:) = nan;
                    traceSamples(eventThresholdTime, eventPlotLogicalCol) = 0;
                    traceSamples(eventThresholdTime, eventPlotEndCol) = nan;
...
%         spikeliststrings = string();
        ...
        for file = 1:(size(filelist,1))
            traceliststrings(size(traceliststrings,2)+1) = convertCharsToStrings(filelist(file).name);
        end
        
        % update list contents in control tab list boxes
        set(mainTraceControl,'Items',traceliststrings);
        set(secondaryTraceControl,'Items',traceliststrings);
        set(analysisList,'Items',matliststrings);
    end
    ...
    % measure the decay phase of the selected event
        ...
        while (traceSamples(maxDecayIdx,traceValueCol)-eventThresholdValue) < (valueAtDesiredDecay-eventThresholdValue)
    ...
    % plots the average event trace
        ...
        if strcmp(traceAlignment,'Align By Threshold')
            for j = 1:size(selectedEvents,1)
                if isnan(selectedEvents(j,averageTraceLogicalCol)) 
                    allTraces = [allTraces nan(totalSamples+1,1)];
                    continue;
                end
        ...
        averageTrace = mean(allTraces,2,'omitnan');
    ...
    % non-existant events are removed from selectedEvents
        ...
        selectedEvents(isnan(selectedEvents(:,eventTimeCol)),:) = nan;
        if ~isnan(selectedEvents(end,eventTimeCol))
            selectedEvents = [selectedEvents; nan(size(selectedEvents))];
        end
    end
